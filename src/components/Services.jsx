...
const Services = ({ content, name, title, img }) => {
  return (
    <div
      className={`relative ${styles.serviceContainer} flex items-center  justify-center`}
    >
      <div
        className={`flex flex-wrap grid-cols-3  md:flex-row items-center h-96 w-96 justify-center ${styles.serviceWrapper}`}
      >
        <div className="space-y-10 ml-20">
          <img
            src={img}
            alt={name}
            className={`w-[80%] h-[60%] md:w-[50%] lg:hidden md:h-auto md:mb-0 md:mr-4 rounded-[20px] ${styles.image} transition-transform transform hover:scale-105`}
          />
        </div>
